...
class CDSRetriever:
    ...
    def __init__(self):
        try:
            ...
            raise CDSRetrievalError(
                f"Failed to initialize CDS API client. Check .cdsapirc file. Error: {e}"
            )

    def retrieve(
        self, dataset_name: str, request_params: Dict, output_path: str
    ) -> str:
        """
        執行單次下載請求。
        """
        ...
        try:
            ...
            if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
                raise CDSRetrievalError(
                    f"Download finished but file is missing or empty: {output_path}"
                )
...
class GribProcessor:
    ...
    def merge_and_convert(self, input_files: List[str], output_nc_path: str) -> str:
        ...
        logger.info(
            f"GribProcessor: Merging {len(input_files)} files into {output_nc_path}..."
        )
        ...
        try:
            ...
            logger.info("GribProcessor: Merge successful.")
...
class DataValidator:
    ...
    def validate_nc(
        self, file_path: str, required_vars: Optional[List[str]] = None
    ) -> xr.Dataset:
            ...
            if required_vars:
                ...
                if missing:
                    raise ValidationError(
                        f"Missing required variables in {file_path}: {missing}"
                    )
...
class ERA5DataManager:
    ...
    def prepare_model_input(
        ...
        sl_request: Dict,
        pl_request: Dict,
        filename_prefix: str = "era5_input",
    ) -> str:
        """
        ...
        try:
            ...
            self.retriever.retrieve(
                "reanalysis-era5-single-levels", sl_request, temp_sl
            )
            ...
            self.retriever.retrieve(
                "reanalysis-era5-pressure-levels", pl_request, temp_pl
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        nc_file = manager.prepare_model_input(
            datetime_str="2023010100", sl_request=sl_params, pl_request=pl_params
        )
