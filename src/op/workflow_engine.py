...
# Import Project-Specific Exceptions for robust error handling
from .era5_reanalysis import DataPipelineError
from .global_operators import GlobalOperatorsError
from .regridders import PreprocessingError
...
class WorkflowEngine:
    ...
    def __init__(self, config_path: str):
        ...
        self.workflow_state: Dict[
            str, Any
        ] = {}  # Stores objects and data produced by steps
    ...
    def _resolve_placeholders(self, value: Any, context: Dict[str, Any]) -> Any:
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                logger.warning(
                    f"Failed to resolve placeholder '{e}' in string '{value}'. It will be kept as is."
                )
                return value
            ...
            return {
                self._resolve_placeholders(k, context): self._resolve_placeholders(
                    v, context
                )
    ...
    def run_workflow(
        self, workflow_name: str, dynamic_context: Optional[Dict[str, Any]] = None
    ):
        ...
        try:
            ...
            if condition:
                ...
                if not evaluated_condition:
                    logger.info(
                        f"Skipping step '{step_name}' due to unmet condition: '{condition}'"
                    )
                    return
            ...
            if instance_key and instance_key in self.workflow_state:
                ...
                raise WorkflowConfigurationError(
                    f"Step '{step_name}' requires 'class' or 'action'."
                )
            ...
            if action_name and action_name != "instantiate":
                ...
                target_obj is not None and not class_name
            ):  # Case of a standalone function directly called
                func_args_config = step_config.get("args", {})
                func_args = self._resolve_placeholders(func_args_config, context)
                result = getattr(module, action_name)(
                    **func_args
                )  # Assuming action_name is the function name

            if output_key:
                self.workflow_state[output_key] = result
                logger.debug(f"Step '{step_name}' stored result in '{output_key}'.")

        except (DataPipelineError, GlobalOperatorsError, PreprocessingError) as e:
            # Handle known domain-specific errors explicitly
            raise WorkflowExecutionError(
                f"Domain error in step '{step_name}': {e}"
            ) from e
        except Exception as e:
            if ignore_errors:
                logger.warning(f"Step '{step_name}' failed but errors are ignored: {e}")
                if output_key:
                    self.workflow_state[output_key] = (
                        None  # Ensure output key exists but is None
                    )
            else:
                raise WorkflowExecutionError(f"Error in step '{step_name}': {e}") from e

    def _execute_iterator_step(
        self, step_config: Dict[str, Any], context: Dict[str, Any]
    ):
        ...
        if source_object_key not in self.workflow_state:
            raise WorkflowConfigurationError(
                f"Iterator step '{step_name}' source object '{source_object_key}' not found in state."
            )
        ...
        logger.info(
            f"Entering iterator loop for '{step_name}' with limit {loop_limit}."
        )
        for i in range(loop_limit):
            try:
                item = next(iterator)
                logger.info(
                    f"Iterator step '{step_name}' - Iteration {i + 1}/{loop_limit}"
                )
                ...
                for inner_step_index, inner_step_config in enumerate(inner_steps):
                    inner_step_name = inner_step_config.get(
                        "name", f"Inner Step {inner_step_index}"
                    )
                    logger.debug(
                        f"  Executing inner step: {inner_step_name} in iteration {i + 1}"
                    )
                ...
                logger.info(
                    f"Iterator for '{step_name}' exhausted after {i} iterations."
                )
                break
            except (GlobalOperatorsError, PreprocessingError) as e:
                # Handle errors occurring during iteration (e.g. InferenceEngine failure)
                raise WorkflowExecutionError(
                    f"Domain error in iterator step '{step_name}' at iteration {i + 1}: {e}"
                ) from e
            except Exception as e:
                raise WorkflowExecutionError(
                    f"Error in iterator step '{step_name}' at iteration {i + 1}: {e}"
                ) from e
...
if __name__ == "__main__":
    ...
    # Create a REAL integrated workflow config file for testing/demonstration
    integrated_config_path = "config/integrated_workflow.yaml"
    os.makedirs("config", exist_ok=True)  # Ensure config directory exists

    # Define a configuration that connects ALL modules: ERA5 -> Regrid -> Model -> Diag -> Export
    integrated_config_content = """
common_settings:
...
workflows:
  full_forecast_cycle:
    ...
    with open(integrated_config_path, "w") as f:
        f.write(integrated_config_content)

    parser = argparse.ArgumentParser(
        description="Run a workflow using the WorkflowEngine."
    )
    parser.add_argument(
        "--config",
        type=str,
        default=integrated_config_path,
        help="Path to the workflow configuration YAML file.",
    )
    parser.add_argument(
        "--workflow",
        type=str,
        default="full_forecast_cycle",
    ...
    try:
        ...
        print("Final workflow state keys:", list(final_state.keys()))
        ...
        era5_dir = resources.get("era5_dir")
        output_base_dir_path = common_settings.get("output_base_dir")

        if era5_dir and os.path.exists(era5_dir):
            import shutil

            shutil.rmtree(era5_dir)
            logger.info(f"Cleaned up ERA5 dir: {era5_dir}")
        ...
        if os.path.exists(integrated_config_path):
            os.remove(integrated_config_path)
            logger.info(f"Cleaned up config file: {integrated_config_path}")
