...
class DataRegridder:
    ...
    def __init__(self, yaml_path):
        ...
        self.base_timestep = timedelta(hours=self.cfg_time["base_step_hours"])
        self.output_timestep = timedelta(hours=self.cfg["regrid"]["output_step_hours"])
    ...
    def build_input_timeline(self):
        ...
        return [self.start_t + t * self.base_timestep for t in range(total_input_steps)]

    def build_output_timeline(self):
        return [
            self.start_t + t * self.output_timestep for t in range(self.total_steps)
        ]
    ...
    def process_single_time(self, curr_time):
        ...
        for (
            var
        ) in self.static.data_vars:  # Iterate over data_vars, not the Dataset itself
            static_data = self.static[var].values
            print(var)
            # Ensure static data is compatible with time dimension
            if (
                static_data.ndim == 2
            ):  # Assuming static variables are (south_north, west_east)
                out_dict[var] = (
        ...
        regrid_nc = output_nc.replace(".nc", "_regrid.nc")
        if self.write_regrid:
            outds.to_netcdf(regrid_nc, format="NETCDF4")
        ...
        for var in ordered_vars:
            if (
                var not in self.diagnostics
            ):  # Skip variables that are just dependencies but not defined as outputs
                continue
            ...
            if all(req in outds.data_vars or req in outds.coords for req in requires):
                ...
                try:
                    ...
                    print(
                        f"[DIAGNOSE] Calculated {var}, shape: {outds[var].shape}"
                    )  # Simpler print
                ...
                missing = [
                    req
                    for req in requires
                    if req not in outds.data_vars and req not in outds.coords
                ]
                print(
    ...
    def temporal_interp(self, time_t, ds1, ds2):
        ...
        for var_name in ds1.data_vars:
            if var_name in ds2.data_vars:
                ...
                interp_data = (
                    ds1[var_name] + (ds2[var_name] - ds1[var_name]) * time_frac
                )
    ...
    def main_process(self):
        ...
        for t_idx, curr_time in enumerate(output_timeline):
            ...
            if not next_input_time_list:
                print(
                    f"[WARN] No next input time found for {curr_time}. End of data range."
                )
            ...
            if not os.path.exists(prev_input_nc) or not os.path.exists(next_input_nc):
                print(
                    f"[WARN] Missing one or both input files for interpolation at {curr_time}. Skipping."
                )
                continue

            # Open datasets
            with (
                xr.open_dataset(prev_input_nc) as ds1,
                xr.open_dataset(next_input_nc) as ds2,
            ):
        ...
        for var in ordered_vars:
            ...
            if all(req in outds.data_vars or req in outds.coords for req in requires):
                ...
                missing = [
                    req
                    for req in requires
                    if not (req in outds.data_vars or req in outds.coords)
                ]
