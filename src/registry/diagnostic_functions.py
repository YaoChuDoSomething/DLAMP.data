...
def _create_dataarray(
    ...
    # Ensure data is a numpy array to satisfy type hints and avoid scalar issues
    ...
    if var_name == "pres_levels":
        ...
        coords["corner_south_north"] = (
            "corner_south_north",
            np.linspace(-450, 450, 451),
        )
    ...
    return xr.DataArray(
        np.expand_dims(data_arr, axis=0).astype(np.float32),
        coords=coords,
        dims=dims,
        name=var_name,
        attrs={
...
def diag_QVAPOR_p(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    ...
    return _create_dataarray(
        data, ds, "QVAPOR_p", "Water Vapor Mixing Ratio", "kg kg-1"
    )
...
def diag_QCLOUD_p(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    ...
    return _create_dataarray(
        data, ds, "QCLOUD_p", "Cloud Water Mixing Ratio", "kg kg-1"
    )
...
def diag_QGRAUP_p(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    ...
    return _create_dataarray(
        data, ds, "QGRAUP_p", "Graupel Water Mixing Ratio", "kg kg-1"
    )
...
def diag_QTOTAL_p(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    ...
    return _create_dataarray(
        data, ds, "QTOTAL_p", "Total Hydrometeors Mixing Ratio", "kg kg-1"
    )
...
def diag_umet100(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    ...
    return _create_dataarray(data, ds, "umet100", "100m U-component of Wind", "m s-1")
...
def diag_vmet100(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    ...
    return _create_dataarray(data, ds, "vmet100", "100m V-component of Wind", "m s-1")
...
def diag_SWDOWN(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    ...
    return _create_dataarray(
        data, ds, "SWDOWN", "Surface Shortwave Downward Radiation", "W m-2"
    )
...
def diag_MAX_REFL(source_dataset: str, ds: xr.Dataset) -> xr.DataArray:
    """
    Maximum Radar Reflectivity (Composite)
    Wrapper for REFL_p as they seem to calculate the same thing (Column Max).
    """
    return diag_REFL_p(source_dataset, ds)
